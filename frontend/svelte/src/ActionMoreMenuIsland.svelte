<script lang="ts">
  ...
  function applyState() {
    ...
    target.querySelectorAll<HTMLElement>('[data-more-menu-dev-only]').forEach((node) => {
    ...
    try {
      ...
      if (deleteTarget && options.targetNode) {
        if (deleteTarget.startsWith('closest ')) {
          options.targetNode.closest(deleteTarget.replace('closest ', ''))?.remove();
        } else {
  ...
  async function openEditModal(editURL: string) {
    ...
    try {
      ...
      document.body.addEventListener('refreshManage', refreshManage, { once: true });
  ...
  function applyHXTrigger(headerValue: string | null) {
    ...
    try {
      ...
      } else if (typeof notifyPayload === 'object' && notifyPayload && typeof (notifyPayload as HXNotify).message === 'string') {
  ...
  function handleReportSubmit(event: SubmitEvent) {
    ...
    const form = eventTarget.closest<HTMLFormElement>('form[data-svelte-report-form]');
  ...
  function handleTargetClick(event: MouseEvent) {
    ...
    const toggleButton = eventTarget.closest<HTMLButtonElement>('[data-more-menu-button]');
    if (toggleButton && target.contains(toggleButton)) {
      toggle();
      return;
    }

    const copyButton = eventTarget.closest<HTMLElement>('[data-more-menu-copy-uri]');
    if (copyButton && target.contains(copyButton)) {
      void copyURI(copyButton.dataset.moreMenuCopyUri || '');
      return;
    }

    const reportButton = eventTarget.closest<HTMLElement>('[data-more-menu-report-dialog]');
    if (reportButton && target.contains(reportButton)) {
      close();
      const dialog = document.getElementById(reportButton.dataset.moreMenuReportDialog || '');
    ...
    const editButton = eventTarget.closest<HTMLElement>('[data-more-menu-edit-url]');
    ...
    const actionButton = eventTarget.closest<HTMLElement>('[data-more-menu-action]');
    if (actionButton && target.contains(actionButton)) {
      ...
      const actionPayload = parsePayload(actionButton.dataset.moreMenuActionPayload || '');
      ...
      const targetNode = actionButton.closest<HTMLElement>('li, button, [data-svelte-action-more-menu]') ?? actionButton;
