<script lang="ts">
  ...
  function modalActionPath() {
    return target.getAttribute('hx-post') || target.getAttribute('hx-put') || '';
  }
  ...
  async function refreshAfterSave(xhr: XMLHttpRequest | undefined) {
    ...
    const select = selectName ? document.querySelector<HTMLSelectElement>(`select[name="${selectName}"]`) : null;
    if (newRKey && select) {
      window.setTimeout(() => {
  ...
  onMount(() => {
    const handleAfterRequest = (event: Event) => {
      ...
      setError(detail?.xhr ? 'Something went wrong. Please try again.' : 'Connection error. Check your network.');
    };
