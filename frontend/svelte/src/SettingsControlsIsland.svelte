<script lang="ts">
  ...
  function applyButtonState() {
    target.querySelectorAll<HTMLButtonElement>('[data-theme-choice]').forEach((button) => {
      const selected = button.dataset.themeChoice === theme;
      button.className = selected ? 'filter-pill-active' : 'filter-pill';
      button.setAttribute('aria-pressed', selected ? 'true' : 'false');
    });

    const checkbox = target.querySelector<HTMLInputElement>('[data-dev-mode-toggle]');
    if (checkbox) {
  ...
  function handleClick(event: MouseEvent) {
    ...
    const button = eventTarget.closest<HTMLButtonElement>('[data-theme-choice]');
  ...
  function handleChange(event: Event) {
    const eventTarget = event.target;
    if (!(eventTarget instanceof HTMLInputElement) || !target.contains(eventTarget)) {
      return;
    }
