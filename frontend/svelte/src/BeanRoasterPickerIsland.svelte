<script lang="ts">
  ...
  function filter() {
    ...
    filtered = allRoasters.filter((roaster) => roaster.name.toLowerCase().includes(q));
  }
...
{#if showDetails}
  <div class="p-3 rounded-lg space-y-2" style="background: var(--surface-bg); border: 1px solid var(--surface-border);">
    ...
    <input
      ...
      class="w-full form-input text-sm"
    />
    <button type="button" onclick={cancelCreate} class="text-xs text-faint hover:text-emphasis">
      Cancel
    </button>
  </div>
{/if}
