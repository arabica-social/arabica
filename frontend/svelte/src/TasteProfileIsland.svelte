<script lang="ts">
  ...
  let points = $derived(axes.map((axis, index) => axisPoint(axis, index, outerRadius)));
  let polygonPoints = $derived(points.map((point) => `${point.x},${point.y}`).join(' '));
  ...
  function readAxesData() {
    const data = document.getElementById('taste-profile-data') as HTMLElement | null;
    ...
    try {
      ...
      axes = parsed.filter((axis) => axis.id && axis.label).map((axis) => ({
        id: axis.id,
        label: axis.label,
...
{#if axes.length > 0 && hasSignals}
  <section class="taste-profile" aria-labelledby="taste-profile-title">
    ...
    <div class="taste-profile-chart">
      <svg viewBox={`0 0 ${size} ${size}`} role="img" aria-labelledby="taste-profile-title taste-profile-desc">
        <desc id="taste-profile-desc">
          A radial taste profile where labels closer to the outside indicate stronger signals.
        </desc>
        <g class="taste-rings">
        ...
        {#each axes as axis, index}
          ...
          <line class="taste-spoke" x1={center} y1={center} x2={end.x} y2={end.y} />
          <text
            class="taste-label"
            x={label.x}
            y={label.y}
        ...
        <polyline class="taste-line" points={`${polygonPoints} ${points[0]?.x || center},${points[0]?.y || center}`} />
        {#each points as point, index}
          <circle class="taste-point" cx={point.x} cy={point.y} r="4.5">
            <title>{axes[index].label}: {axes[index].value}/100, {axes[index].evidence}</title>
          </circle>
        {/each}
      </svg>
    </div>
