<script lang="ts">
  ...
  function sessionModal() {
    return document.getElementById('session-expired-modal') as HTMLDialogElement | null;
  }
  ...
  function showSessionExpiredModal() {
    ...
    const returnInput = document.getElementById('reauth-return-to') as HTMLInputElement | null;
    ...
    try {
      ...
      const interval = setInterval(() => {
        ...
        for (const [key, value] of Object.entries(formData)) {
          const el = form.querySelector<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(`[name="${CSS.escape(key)}"]`);
          if (!el) continue;
          if (el instanceof HTMLInputElement && (el.type === 'checkbox' || el.type === 'radio')) {
  ...
  function formatLocalTimes(root: ParentNode = document) {
    root.querySelectorAll<HTMLTimeElement>('time[data-local]').forEach((el) => {
      ...
      try {
        if (fmt === 'date') {
          text = dt.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
        } else if (fmt === 'year') {
  ...
  function cleanHistorySnapshot() {
    document.querySelectorAll<HTMLElement>('main, main *, body').forEach((el) => {
  ...
  function extractMessage(detail: any) {
    if (!detail) return '';
    if (detail.value && typeof detail.value === 'object') return detail.value.message || '';
  ...
  function handleHTMXConfigRequest(event: Event) {
    ...
    const traceparent = document.querySelector<HTMLMetaElement>('meta[name="traceparent"]')?.content;
  ...
  function handleCloseDialog(event: Event) {
    const value = (event as CustomEvent<{ value?: string | { id?: string; delay?: number } }>).detail?.value;
    if (!value) return;
  ...
  onMount(() => {
    ...
    document.body.addEventListener('htmx:beforeHistorySave', cleanHistorySnapshot);
    document.body.addEventListener('close-dialog', handleCloseDialog);
    window.addEventListener('notify', handleNotify);

    return () => {
      if (window.__showSessionExpiredModal === showSessionExpiredModal) delete window.__showSessionExpiredModal;
      document.body.removeEventListener('click', handleClick);
      document.body.removeEventListener('submit', handleSubmit);
      document.body.removeEventListener('htmx:configRequest', handleHTMXConfigRequest);
      ...
      document.body.removeEventListener('htmx:beforeHistorySave', cleanHistorySnapshot);
