<script lang="ts">
  ...
  function applyTabs() {
    ...
    target.querySelectorAll<HTMLButtonElement>('[data-admin-tab]').forEach((button) => {
    ...
    target.querySelectorAll<HTMLElement>('[data-admin-panel]').forEach((panel) => {
  ...
  function setAddLabelOpen(open: boolean) {
    const formWrap = target.querySelector<HTMLElement>('[data-admin-add-label-form]');
    if (formWrap) {
  ...
  function setCopied(button: HTMLButtonElement | null) {
    if (copiedButton) {
      copiedButton.title = copiedButton.dataset.copyTitle || 'Copy to clipboard';
      copiedButton.querySelectorAll<HTMLElement>('[data-copy-idle]').forEach((node) => {
        node.hidden = false;
      });
      copiedButton.querySelectorAll<HTMLElement>('[data-copy-copied]').forEach((node) => {
        node.hidden = true;
      });
    }
    copiedButton = button;
    ...
    button.querySelectorAll<HTMLElement>('[data-copy-copied]').forEach((node) => {
      node.hidden = false;
    });
  ...
  function handleClick(event: MouseEvent) {
    ...
    const tabButton = eventTarget.closest<HTMLButtonElement>('[data-admin-tab]');
    ...
    const copyButton = eventTarget.closest<HTMLButtonElement>('[data-copy-button]');
