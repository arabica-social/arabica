...
function loadIsland(id: string, importer: () => Promise<IslandModule>): Promise<any> {
...
async function mountFeedFilters() {
	...
	const FeedFiltersIsland = await loadIsland('feed-filters', () => import('./FeedFiltersIsland.svelte') as Promise<IslandModule>);
...
async function mountRecipeExplore() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-recipe-explore]');
	if (targets.length === 0) {
		return;
	}
...
async function mountManageTabs() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-manage-tabs]');
	if (targets.length === 0) {
		return;
	}
...
async function mountCommentSections() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-comment-section]');
	if (targets.length === 0) {
		return;
	}
...
async function mountEntitySuggests() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-entity-suggest]');
	if (targets.length === 0) {
		return;
	}
...
async function mountModalShells() {
	const targets = document.querySelectorAll<HTMLFormElement>('form[data-svelte-modal-shell]');
	if (targets.length === 0) {
		return;
	}
...
async function mountOolongForms() {
	const targets = document.querySelectorAll<HTMLFormElement>('form[data-svelte-oolong-form]');
	if (targets.length === 0) {
		return;
	}
...
async function mountBeanRoasterPickers() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-bean-roaster-picker]');
	if (targets.length === 0) {
		return;
	}
...
async function mountBeanRatings() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-bean-rating], [data-svelte-rating-toggle]');
	if (targets.length === 0) {
		return;
	}
	const BeanRatingIsland = await loadIsland('bean-rating', () => import('./BeanRatingIsland.svelte') as Promise<IslandModule>);
	targets.forEach((target) => {
...
async function mountBeanViewActions() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-bean-view-actions]');
	if (targets.length === 0) {
		return;
	}
...
async function mountDisclosures() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-disclosure]');
	if (targets.length === 0) {
		return;
	}
...
async function mountShareButtons() {
	const targets = document.querySelectorAll<HTMLButtonElement>('button[data-svelte-share]');
	if (targets.length === 0) {
		return;
	}
...
async function mountActionMoreMenus() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-action-more-menu]');
	if (targets.length === 0) {
		return;
	}
...
async function mountCommentReplies() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-comment-reply]');
	if (targets.length === 0) {
		return;
	}
...
async function mountSettingsControls() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-settings-controls]');
	if (targets.length === 0) {
		return;
	}
...
async function mountSettingsForms() {
	const targets = document.querySelectorAll<HTMLFormElement>('[data-svelte-settings-form]');
	if (targets.length === 0) {
		return;
	}
...
async function mountScrollTopControls() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-scroll-top]');
	if (targets.length === 0) {
		return;
	}
...
async function mountAdminDashboards() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-admin-dashboard]');
	if (targets.length === 0) {
		return;
	}
...
async function mountSaveAsRecipeControls() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-save-as-recipe]');
	if (targets.length === 0) {
		return;
	}
...
async function mountRecipeForkButtons() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-recipe-fork]');
	if (targets.length === 0) {
		return;
	}
...
async function mountHandleAutocompletes() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-handle-autocomplete]');
	if (targets.length === 0) {
		return;
	}
...
async function mountProfileStats() {
	const targets = document.querySelectorAll<HTMLElement>('[data-svelte-profile-stats]');
	if (targets.length === 0) {
		return;
	}
...
async function mountBrewForms() {
	...
	const BrewFormIsland = await loadIsland('brew-form', () => import('./BrewFormIsland.svelte') as Promise<IslandModule>);
	targets.forEach((target) => {
...
async function mountFeedLayout() {
	...
	const hasFeedLayout =
		document.querySelector('[data-feed-masonry], #feed-items, [data-svelte-feed-filters]') !== null;
	if (!hasFeedLayout) {
		return;
	}

	const FeedMasonryIsland = await loadIsland('feed-masonry', () => import('./FeedMasonryIsland.svelte') as Promise<IslandModule>);
...
async function mountCoreIslands() {
	...
	const [AppCacheRuntimeIsland, GlobalActionsIsland, LayoutRuntimeIsland, ThemeRuntimeIsland, TransitionRuntimeIsland, ServiceWorkerIsland] =
		await Promise.all([
			loadIsland('app-cache-runtime', () => import('./AppCacheRuntimeIsland.svelte') as Promise<IslandModule>),
			loadIsland('global-actions', () => import('./GlobalActionsIsland.svelte') as Promise<IslandModule>),
			loadIsland('layout-runtime', () => import('./LayoutRuntimeIsland.svelte') as Promise<IslandModule>),
			loadIsland('theme-runtime', () => import('./ThemeRuntimeIsland.svelte') as Promise<IslandModule>),
			loadIsland('transition-runtime', () => import('./TransitionRuntimeIsland.svelte') as Promise<IslandModule>),
			loadIsland('service-worker', () => import('./ServiceWorkerIsland.svelte') as Promise<IslandModule>)
		]);
