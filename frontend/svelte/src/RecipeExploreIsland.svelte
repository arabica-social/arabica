<script lang="ts">
  ...
  async function search() {
    loading = true;
    try {
      const response = await fetch(`/api/recipes/suggestions?${searchParams()}`, {
  ...
  function handleRecipeCardKeydown(event: KeyboardEvent, recipe: ExploreRecipe) {
  ...
  function notify(message: string) {
    window.dispatchEvent(new CustomEvent('notify', { detail: { message }, bubbles: true }));
  }
  ...
  async function forkRecipe() {
    ...
    try {
      ...
      notify(`Failed to copy recipe: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
<div class="flex items-center justify-between mb-3">
  {#if !loading && recipes.length > 0}
    <p class="text-sm text-muted">{recipes.length} recipe{recipes.length === 1 ? '' : 's'} found</p>
  {:else}
    <div></div>
  {/if}
    ...
    <div class="card card-inner">
      <div class="flex justify-between items-start mb-4">
        <div class="min-w-0 flex-1">
          <h3 class="text-xl font-bold text-primary wrap-break-word">{selectedRecipe.name}</h3>
          <a href={`/profile/${selectedRecipe.author_did}`} class="flex items-center gap-2 mt-1 group/author">
            ...
            <div>
              {#if selectedRecipe.author_display}
                <span class="block text-sm font-medium text-emphasis group-hover/author:text-primary group-hover/author:underline transition-colors">
      ...
      {#if selectedRecipe.source_ref}
        <p class="text-sm text-faint mb-3">
          ...
          {#if selectedRecipe.source_author_display || selectedRecipe.source_author_handle}
            <a href={sourceRecipeURL(selectedRecipe)} class="text-emphasis underline hover:text-primary">
      ...
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
        ...
        {#if isAuthenticated && !isOwner(selectedRecipe)}
          <button type="button" onclick={forkRecipe} class="btn-secondary text-sm">Copy Recipe</button>
        {/if}
        <a href={`/recipes/${ownerForURL(selectedRecipe)}/${selectedRecipe.rkey}`} class="btn-secondary text-sm text-center">View Recipe</a>
      </div>
    </div>
  </div>
{/if}
...
<div>
  {#if loading}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each [0, 1, 2] as index}
        <div class={`feed-card animate-pulse ${index === 1 ? 'hidden sm:block' : ''} ${index === 2 ? 'hidden lg:block' : ''}`}>
  ...
  {:else}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each recipes as recipe}
        ...
        >
          <a href={`/profile/${recipe.author_did}`} class="flex items-center gap-2 mb-3 group/author" onclick={(event) => event.stopPropagation()}>
            {#if recipe.author_avatar}
              <img src={recipe.author_avatar} class="w-7 h-7 rounded-full object-cover" alt={authorName(recipe)} loading="lazy" width="28" height="28" />
            {:else}
            ...
            <span class="min-w-0 flex-1">
              {#if recipe.author_display}
                <span class="block truncate text-sm font-medium text-emphasis group-hover/author:text-primary group-hover/author:underline transition-colors">
