...
`fix:`, `refactor:`, `chore:`, optionally scoped like `feat(oolong): …`). Keep
messages to the subject line only — do NOT add a long description unless there
is something genuinely important to note (e.g. lexicon schema changes, breaking
changes, non-obvious migrations).
...
- **Storage:** AT Protocol PDS (user data), BoltDB (sessions), SQLite (firehose
  index)
...
All tests MUST use [testify/assert](https://github.com/stretchr/testify). Do NOT
use `if` statements with `t.Error()`.
...
5. **Respect the ritual** — No urgency, no gamification, intentional
   interactions
