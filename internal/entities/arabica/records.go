...
// RecordToRecipe converts an atproto record map to a Recipe
func RecordToRecipe(record map[string]any, atURI string) (*Recipe, error) {
	...
	if poursRaw, ok := record["pours"].([]any); ok {
		recipe.Pours = make([]*Pour, len(poursRaw))
...
// RecordToBean converts an atproto record map to a Bean
...
// RecordToRoaster converts an atproto record map to a Roaster
...
// RecordToGrinder converts an atproto record map to a Grinder
...
// RecordToBrewer converts an atproto record map to a Brewer
