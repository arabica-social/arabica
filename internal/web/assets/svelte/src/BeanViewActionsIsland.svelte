<script lang="ts">
  ...
  async function patchBean(
    overrides: Partial<BeanPayload>,
    errorMessage: string,
    action: "close" | "save" | "remove",
  ) {
    if (savingAction) {
      return;
    }
    ...
    <div class="modal-content">
      ...
      <div class="flex gap-2">
        <button
          type="button"
          onclick={() =>
            patchBean({ closed: true }, "Failed to close bag", "close")}
...
>
  <div class="modal-content">
    ...
    <div class="space-y-4">
      ...
      />
      <div
        class="text-center text-3xl font-bold text-secondary"
        aria-hidden="true"
      >
        {rating}/10
      </div>
    ...
    <div class="flex gap-2 mt-4">
      ...
      {#if hasRating}
        <button
          type="button"
          onclick={() =>
            patchBean({ rating: null }, "Failed to remove rating", "remove")}
