<script lang="ts">
  ...
  function clearStationDrawers(event: MouseEvent) {
    ...
    const addButton = target.closest(
      ".station-add[hx-target^='#station-drawer-slot-']",
    );
    if (!addButton) return;
    document
      .querySelectorAll<HTMLElement>(".station-drawer-row")
      .forEach((slot) => {
        slot.innerHTML = "";
      });
  }
  ...
  function triggeringElement(event: Event) {
    const detail = (
      event as CustomEvent<{ elt?: unknown; requestConfig?: { elt?: unknown } }>
    ).detail;
  ...
  function handleHTMXAfterRequest(event: Event) {
    ...
    const redirect = trigger?.closest<HTMLElement>("[data-delete-redirect]")
      ?.dataset.deleteRedirect;
