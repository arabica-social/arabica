<script lang="ts">
  ...
  function setActiveTab(tab: string) {
    ...
    const tabs = Array.from(
      target.querySelectorAll<HTMLElement>("[data-manage-tab]"),
    ).map((button) => button.dataset.manageTab || "");
  ...
  function refreshSyncButton() {
    target
      .querySelectorAll<HTMLButtonElement>("[data-manage-refresh]")
      .forEach((button) => {
        button.disabled = syncing;
        button
          .querySelector("[data-sync-idle]")
          ?.toggleAttribute("hidden", syncing);
        button
          .querySelector("[data-sync-busy]")
          ?.toggleAttribute("hidden", !syncing);
        button
          .querySelector("[data-sync-icon]")
          ?.classList.toggle("animate-spin", syncing);
      });
  }
  ...
  onMount(() => {
    const storageKey = target.dataset.tabStorageKey || "";
    const initialTab =
      (storageKey && readStoredTab(storageKey)) ||
      target.dataset.initialTab ||
      "brews";
    ...
    const handleClick = (event: Event) => {
      ...
      const refreshButton = node?.closest<HTMLButtonElement>(
        "[data-manage-refresh]",
      );
