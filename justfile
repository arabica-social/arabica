...
run:
    @LOG_LEVEL=debug LOG_FORMAT=console ARABICA_MODERATORS_CONFIG=roles.json ARABICA_HOTRELOAD=1 go run ./cmd/server -known-dids known-dids.txt

templ-watch:
    @templ generate --watch --proxy="http://localhost:18080" --cmd="ARABICA_HOTRELOAD=1 go run ./cmd/server -known-dids known-dids.txt"
