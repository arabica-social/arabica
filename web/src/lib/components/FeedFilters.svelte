...
<div class="mb-5 flex flex-wrap items-center justify-between gap-2" aria-busy={loading}>
	<div class="flex flex-wrap gap-1" role="group" aria-label="Feed filters">
		{#each activeTabs as tab (tab.value)}
			<StampTag label={tab.label} active={typeFilter === tab.value} tone={tab.value} disabled={loading} onclick={() => onType(tab.value)} />
		{/each}
	</div>
	<div class="flex items-center gap-1 flex-shrink-0">
		<span class="sort-label">Sort</span>
		<StampTag label="New" active={sortActive("recent")} disabled={loading} onclick={() => onSort("recent")} />
		<StampTag label="Popular" active={sortActive("popular")} disabled={loading} onclick={() => onSort("popular")} />
	</div>
</div>

<style>
	.sort-label { margin-right: .2rem; color: var(--text-faint); font-size: .62rem; font-weight: 600; letter-spacing: .12em; text-transform: uppercase; }
</style>
