...
describe("Recipes catalog page", () => {
  ...
  it("renders LedgerHeader with h1, eyebrow, description, and New Recipe action", () => {
    ...
    expect(
      screen.getByText(
        "Browse, compare, and fork brewing recipes shared by the Arabica community.",
      ),
    ).toBeTruthy();
  ...
  it("shows the alpha disclosure as a native details/summary with expanded explanation", async () => {
    ...
    const disclosure = document.querySelector(
      "details.recipe-alpha-notice",
    ) as HTMLDetailsElement | null;
    expect(disclosure).toBeTruthy();
    const summary = screen.getByText("Alpha format").closest("summary");
    expect(summary).toBeTruthy();
    expect(disclosure?.open).toBe(false);
    await user.click(summary!);
    expect(disclosure?.open).toBe(true);
    expect(
      screen.getByText(/recipe format may change significantly/),
    ).toBeTruthy();
  });
  ...
  it("renders search, category filters, brewer type, and amount controls", () => {
    ...
    expect(
      screen.getByRole("group", { name: "Recipe size filters" }),
    ).toBeTruthy();
  ...
  it("shows empty-state messaging when no recipes match", () => {
    render(RecipesCatalog, {
      data: { recipes: [], error: "", isAuthenticated: true },
    });
